...
<form action="{{route('admin.products.update', ['product' => $product->id])}}" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <select name="categories[]" id="" class="form-control" multiple>
            ...
            <option value="{{ $categoria->id }}" @if($product->categories->contains($categoria)) selected @endif
...
<div class="row">
    ...
    <div class="col-4 text-center">
        ...
        <form action="{{ route('admin.photo.remove', ['photoName' => $photo->image])}}" method="post">
            @csrf
            <input type="hidden" value="{{$photo->image}}" name="photoName">
            <button type="submit" class="btn btn-lg btn-danger">deletar</button>
        </form>
    </div>
    @endforeach
</div>
